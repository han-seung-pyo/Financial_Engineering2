...
#만기 때 페이오프
value_matrix[s0_node:,-1] = FV *(1+cpn_rate)
for i in range(barrier_node,s0_node):
    value_matrix[i,-1] = FV*(cpn_rate + (i/s0_node))
    
for i in range(barrier_node):
    value_matrix[i,-1] = (i/s0_node)*FV
#lower boundary
value_matrix[0,:]= 0
#upper boundary
indexing = 0
...
#%%
value_matrix_no = np.zeros((jmax,imax+1))
value_matrix[barrier_node,-1] = 
